...
def add_chroma_id_column():
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(text("""
            ...
            WHERE table_name = 'grant_contracts' 
            AND column_name = 'chroma_id'
        """))
        
        if not result.fetchone():
            print("Adding chroma_id column to contracts table...")
            conn.execute(text("ALTER TABLE grant_contracts ADD COLUMN chroma_id VARCHAR"))
            conn.commit()
        ...
        if result.fetchone():
            ...
            conn.execute(text("ALTER TABLE grant_contracts DROP COLUMN embedding"))
            conn.commit()
