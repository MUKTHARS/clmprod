...
function ContractDetailsPage({ user = null }) {
  ...
  // Helper functions
  const getSingleObjective = () => {
    if (!contractData?.comprehensive_data?.contract_details) return "";
    
    const compData = contractData.comprehensive_data;
    const contractDetails = compData.contract_details || {};
    
    // If single_objective is provided
    if (contractDetails.single_objective) {
      return contractDetails.single_objective;
    }
    
    // If objectives is an array, combine them
    if (contractDetails.objectives && Array.isArray(contractDetails.objectives)) {
      const objectives = contractDetails.objectives
        .filter(obj => obj && typeof obj === 'string')
        .map(obj => obj.trim())
        .filter(obj => obj.length > 0);
      
      if (objectives.length === 0) return "";
      
      // Combine into single objective
      const combined = objectives.join('. ');
      return combined.length > 500 ? combined.substring(0, 497) + '...' : combined;
    }
    
    // If objectives is a string
    if (typeof contractDetails.objectives === 'string') {
      return contractDetails.objectives;
    }
    
    return contractDetails.purpose || "";
  };

  const getTermsSummary = () => {
    if (!contractData?.comprehensive_data?.terms_conditions) {
      return "Standard terms and conditions apply as per the agreement.";
    }
    ...
    if (terms.intellectual_property && terms.intellectual_property !== "Not specified") {
      keyClauses.push("intellectual property rights");
    }
    if (terms.confidentiality && terms.confidentiality !== "Not specified") {
      keyClauses.push("confidentiality obligations");
    }
    if (terms.liability && terms.liability !== "Not specified") {
      keyClauses.push("liability limitations");
    }
    if (terms.termination_clauses && terms.termination_clauses !== "Not specified") {
      keyClauses.push("termination conditions");
    }
    
    if (keyClauses.length > 0) {
      return `Key terms include ${keyClauses.join(', ')} as specified in the agreement.`;
    }
  ...
  const normalizeContractData = (apiResponse) => {
    ...
    if (compData && typeof compData === 'object') {
      ...
      let singleObjective = "";
      if (contractDetails.objectives && Array.isArray(contractDetails.objectives)) {
        // If objectives is an array, combine into single objective
        singleObjective = contractDetails.objectives
          .map(obj => {
            if (typeof obj !== 'string') return String(obj);
            let cleaned = obj
              .replace(/^["'`]+|["'`]+$/g, '')
          ...
          .filter(obj => obj && obj.trim().length > 2)
          .join('. ');  // Join all objectives into a single string
        
        // If it's too long, take the first one or summarize
        if (singleObjective.length > 500) {
          singleObjective = singleObjective.substring(0, 497) + '...';
        }
      } else if (contractDetails.single_objective) {
        // If single_objective is provided directly
        singleObjective = contractDetails.single_objective;
      } else if (contractDetails.objectives && typeof contractDetails.objectives === 'string') {
        // If objectives is already a string
        singleObjective = contractDetails.objectives;
      }
      
      // Handle terms & conditions summary
      let termsSummary = "";
      if (terms.summary) {
        termsSummary = terms.summary;
      } else {
        // If no summary provided, create one from detailed clauses
        const termDetails = [];
        if (terms.intellectual_property && terms.intellectual_property !== "Not specified") {
          termDetails.push("Intellectual property rights");
        }
        if (terms.confidentiality && terms.confidentiality !== "Not specified") {
          termDetails.push("confidentiality obligations");
        }
        if (terms.liability && terms.liability !== "Not specified") {
          termDetails.push("liability limitations");
        }
        // Add other key clauses as needed
        
        if (termDetails.length > 0) {
          termsSummary = `Key terms include ${termDetails.join(', ')}.`;
        } else {
          termsSummary = "Standard terms and conditions apply as per the agreement.";
        }
      }
      ...
      const normalized = {
        ...
        comprehensive_data: {
          contract_details: {
            ...contractDetails,
            single_objective: singleObjective  
          },
          parties: parties,
          ...
          <div className="expandable-section">
            ...
            {expandedSections.contractDetails && (
              <div className="expandable-content">
                ...
                {/* Single Objective - Full width */}
                {getSingleObjective() && (
                  <div className="field-card full-width">
                    <div className="field-header">
                      <Target size={16} />
                      <label className="field-label">Project Objective</label>
                    </div>
                    <div className="field-value-container">
                      <span className="field-value paragraph-value">
                        {getSingleObjective()}
...
// {compData.contract_details?.single_objective && (
//   <div className="field-card full-width">
//     <div className="field-header">
//       <Target size={16} />
//       <label className="field-label">Project Objective</label>
