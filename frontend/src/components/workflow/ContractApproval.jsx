...
function ContractApproval() {
  ...
  const fetchContractsForApproval = async () => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/contracts/status/reviewed`, {
        headers: {
  ...
  const handleFinalApproval = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/contracts/${selectedContract.id}/final-approval`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
